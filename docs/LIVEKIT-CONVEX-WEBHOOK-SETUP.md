...
1. Open **Convex Dashboard** → your project → **Production** deployment (**patient-crocodile-0**) → **Settings** → **Environment variables**.
...
## 2. LiveKit webhook URL (Coolify) — done

- **`LIVEKIT_WEBHOOK_URL`** has been set on the **livekit-stack** app in Coolify to:
  **`https://patient-crocodile-0.eu-west-1.convex.site/livekit-webhook`**
- A **redeploy** of the LiveKit Stack has been triggered so the server picks up the new env and `livekit.yaml` webhook config. Check Coolify for deployment status.

Your Convex HTTP base URL is **`https://patient-crocodile-0.eu-west-1.convex.site`** (production). The repo’s `deploy/livekit.yaml` includes a `webhook` section that uses `LIVEKIT_WEBHOOK_URL`.
...
| **Coolify** LiveKit Stack app env | `LIVEKIT_WEBHOOK_URL` = `https://patient-crocodile-0.eu-west-1.convex.site/livekit-webhook` |
