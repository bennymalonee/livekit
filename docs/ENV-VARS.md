...
- **Convex Dashboard (by document):** [Dashboard](https://dashboard.convex.dev) → your deployment → **Data** → **users** → open a user document → set `role` to `"admin"` or `"operator"` (leave blank or `"viewer"` for read-only).
- **Convex Dashboard (by email, first admin only):** **Functions** → **Run** → choose `rbac:bootstrapSetRoleByEmail` → args: `{"email": "your@email.com", "role": "admin"}` → Run. Only works when no admin exists yet.
